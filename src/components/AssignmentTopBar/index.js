...
const AssignmentTopBar = ({classroom,class_id}) => {
  ...
  return (
    ...
    {classroom?.role!=="student"?<div className='assignment_top_bar'>
        <div className='tabs'>
          <Link to={"/app/classroom/"+class_id+"/assignment/"+assignment_id} className={pathname.split("/").length===6?"tab active":"tab"	}>
              <i className="fa-regular fa-book"></i> Assignment
          </Link>
          <Link to={"/app/classroom/"+class_id+"/assignment/"+assignment_id+"/submissions"} className={pathname.split("/").length===7?"tab active":"tab"}>
